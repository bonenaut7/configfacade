...
public final class ConfigArray extends ConfigElement implements Iterable<ConfigElement> {
	...
	public ConfigArray getArrayAt(int index) {
		if (index > -1 && index < this.list.size()) {
			final ConfigElement element = this.list.get(index);
			return element instanceof ConfigArray ? (ConfigArray)element : null;
		}
	...
	public ConfigCompound getCompoundAt(int index) {
		if (index > -1 && index < this.list.size()) {
			final ConfigElement element = this.list.get(index);
			return element instanceof ConfigCompound ? (ConfigCompound)element : null;
		}
		return null;
	}
	...
	public ConfigPrimitive getPrimitiveAt(int index) {
		if (index > -1 && index < this.list.size()) {
			final ConfigElement element = this.list.get(index);
			return element instanceof ConfigPrimitive ? (ConfigPrimitive)element : null;
		}
